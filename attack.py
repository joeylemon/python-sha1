...
def extend(m: str, m_malicious: str, MAC: int) -> (int, str):
    """
    ...
    __print("Mallory's calculated m_prime:")
    __print_hex(m_prime)
...
def run(S: str, m: str, m_malicious: str) -> (str, str):
    """
    ...
    __print("Bob will calculate SHA1 of:")
    __print_hex(concatenate(sha.encode_string(S), m_prime))

    # MAC' should be equal to Bob's MAC
    return m_prime, MAC_prime, MAC_b


def __print_hex(val: int) -> None:
    """
...
if __name__ == "__main__":
    ...
    m_prime, MAC_prime, HMAC = run(args.secret, args.message, args.malicious)

    print("Alice calculates MAC as SHA1(S || m)")
    print(f"S=0x{"".join(f'{ord(c)}:02x' for c in S)}")
    print(f"m=0x{args.message:x}")
    print(f"MAC=0x{sha.sha1(sha.encode_string(args.secret + args.message))}")

    print("\nMallory extends m to create m'")
    print(f"m'=0x{m_prime:x}")

    print(f"Mallory's MAC 0x{MAC_prime}")
    print(f"Bob's MAC     0x{HMAC}")

    print("\nMallory sends m' to Bob")
    print(f"m'={hex(m_prime)}")
